...
impl Phoneme {
    ...
    fn from_label(label: u32) -> Option<Self> {
        if label >= 1000 && label < 1100 {
            match label - 1000 {
                0 => Some(Phoneme::AA), 1 => Some(Phoneme::AE), 2 => Some(Phoneme::AH),
                3 => Some(Phoneme::AO), 4 => Some(Phoneme::AW), 5 => Some(Phoneme::AY),
                6 => Some(Phoneme::EH), 7 => Some(Phoneme::ER), 8 => Some(Phoneme::EY),
                9 => Some(Phoneme::IH), 10 => Some(Phoneme::IY), 11 => Some(Phoneme::OW),
                12 => Some(Phoneme::OY), 13 => Some(Phoneme::UH), 14 => Some(Phoneme::UW),
                15 => Some(Phoneme::B), 16 => Some(Phoneme::CH), 17 => Some(Phoneme::D),
                18 => Some(Phoneme::DH), 19 => Some(Phoneme::F), 20 => Some(Phoneme::G),
                21 => Some(Phoneme::HH), 22 => Some(Phoneme::JH), 23 => Some(Phoneme::K),
                24 => Some(Phoneme::L), 25 => Some(Phoneme::M), 26 => Some(Phoneme::N),
                27 => Some(Phoneme::NG), 28 => Some(Phoneme::P), 29 => Some(Phoneme::R),
                30 => Some(Phoneme::S), 31 => Some(Phoneme::SH), 32 => Some(Phoneme::T),
                33 => Some(Phoneme::TH), 34 => Some(Phoneme::V), 35 => Some(Phoneme::W),
                36 => Some(Phoneme::Y), 37 => Some(Phoneme::Z), 38 => Some(Phoneme::ZH),
                39 => Some(Phoneme::SIL),
                _ => None,
            }
        } else {
    ...
    fn to_string(&self) -> &'static str {
        match self {
            ...
            Phoneme::DH => "DH", Phoneme::F => "F", Phoneme::G => "G",
            Phoneme::HH => "HH", Phoneme::JH => "JH", Phoneme::K => "K",
...
fn build_g2p_rules() -> VectorFst<TropicalWeight> {
    ...
    let rules = vec![
        ('a', Phoneme::AE), ('e', Phoneme::EH), ('i', Phoneme::IH),
        ('o', Phoneme::AO), ('u', Phoneme::AH), ('b', Phoneme::B),
        ('c', Phoneme::K), ('d', Phoneme::D), ('f', Phoneme::F),
        ('g', Phoneme::G), ('h', Phoneme::HH), ('j', Phoneme::JH),
        ('k', Phoneme::K), ('l', Phoneme::L), ('m', Phoneme::M),
        ('n', Phoneme::N), ('p', Phoneme::P), ('r', Phoneme::R),
        ('s', Phoneme::S), ('t', Phoneme::T), ('v', Phoneme::V),
        ('w', Phoneme::W), ('y', Phoneme::Y), ('z', Phoneme::Z),
    ];
...
fn extract_phonemes_from_state(fst: &VectorFst<TropicalWeight>, state: u32, phonemes: &mut Vec<Phoneme>) {
...
fn main() -> Result<()> {
    ...
    for word in test_words {
        ...
        if let Some(pronunciations) = lookup_word_in_lexicon(&lexicon_entries, word) {
            for (i, pron) in pronunciations.iter().enumerate() {
                let phoneme_str: Vec<&str> = pron.iter()
                    .map(|p| p.to_string())
                    .collect();
    ...
    for word in unknown_words {
        ...
        if !phonemes.is_empty() {
            let phoneme_str: Vec<&str> = phonemes.iter()
                .map(|p| p.to_string())
                .collect();
            ...
            for (i, pron) in entry.pronunciations.iter().enumerate() {
                ...
                println!("    {}: {} ({})", 
                         ...
                         match entry.word.as_str() {
                             "hello" => if i == 0 { "standard" } else { "variant" },
                             "read" => if i == 0 { "present tense" } else { "past tense" },
                             "live" => if i == 0 { "verb /lɪv/" } else { "adjective /laɪv/" },
                             "the" => if i == 0 { "unstressed /ðə/" } else { "stressed /ðiː/" },
                             _ => "variant",
                         });
            }
            println!();
