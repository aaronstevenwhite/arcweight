        ...
        for state in det.states() {
            ...
            for arc in det.arcs(state) {
                assert!(seen_labels.insert(arc.ilabel), 
                    "Found duplicate input label {} from state {}", arc.ilabel, state);
            }
        }
...
mod shortest_path_tests {
    ...
    fn test_shortest_path_multiple() {
        ...
        let config = ShortestPathConfig { nshortest: 2, ..Default::default() };
    ...
    fn test_shortest_path_empty() {
        ...
        match shortest_path_single::<TropicalWeight, VectorFst<TropicalWeight>, VectorFst<TropicalWeight>>(&fst) {
...
mod epsilon_removal_tests {
    ...
    fn test_remove_epsilons() {
        ...
        let no_eps: VectorFst<BooleanWeight> = remove_epsilons::<BooleanWeight, VectorFst<BooleanWeight>, VectorFst<BooleanWeight>>(&bool_fst).unwrap();
...
mod topsort_tests {
        ...
        for state in sorted.states() {
            for arc in sorted.arcs(state) {
                assert!(state < arc.nextstate, 
                    "Arc from {} to {} violates topological order", state, arc.nextstate);
            }
        }
    ...
    fn test_topsort_cyclic() {
        ...
        let result = topsort::<TropicalWeight, VectorFst<TropicalWeight>, VectorFst<TropicalWeight>>(&fst);
