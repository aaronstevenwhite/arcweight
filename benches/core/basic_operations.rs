...
fn bench_determinize(c: &mut Criterion) {
    ...
    group.bench_function("linear_100", |b| {
        b.iter(|| {
            let result: VectorFst<TropicalWeight> = determinize(black_box(&linear_fst)).unwrap();
            black_box(result);
        })
    });
    ...
    group.bench_function("branching_50", |b| {
        b.iter(|| {
            let result: VectorFst<TropicalWeight> = determinize(black_box(&branching_fst)).unwrap();
            black_box(result);
        })
    });
