...
fn main() -> Result<()> {
    ...
    for (name, ins, del, sub) in configs {
        ...
        if distance.is_finite() {
            println!("  {}: {} (ins={}, del={}, sub={})", name, distance, ins, del, sub);
        } else {
            println!("  {}: No transformation found (ins={}, del={}, sub={})", name, ins, del, sub);
        }
    }
    ...
    for (source, target, description) in examples {
        let distance = compute_edit_distance(source, target, 1.0, 1.0, 1.0)?;
        println!("  '{}' -> '{}': {} (distance: {})", source, target, description, distance);
    }
