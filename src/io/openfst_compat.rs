    ...
    Writer: Write + Seek,
{
    ...
    writer.write_all(b"vector\0\0")?; // fst type
    writer.write_all(b"tropical\0\0\0\0\0\0\0\0")?; // arc type
    ...
    writer.write_i64::<LittleEndian>(0)?; // num arcs (filled later)
    ...
    // update arc count
    writer.seek(std::io::SeekFrom::Start(48))?;
