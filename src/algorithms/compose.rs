...
where
    ...
    CF: ComposeFilter<W>,
{
    let start1 = fst1.start().ok_or_else(|| {
        Error::Algorithm("First FST has no start state".into())
    })?;
    let start2 = fst2.start().ok_or_else(|| {
        Error::Algorithm("Second FST has no start state".into())
    })?;
