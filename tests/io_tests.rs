...
mod text_format_tests {
    ...
    fn test_write_read_text_roundtrip() {
        ...
        let read_fst: VectorFst<TropicalWeight> = read_text::<TropicalWeight, VectorFst<TropicalWeight>, _>(&mut buf_reader, None, None).unwrap();
    ...
    fn test_write_read_empty_fst() {
        ...
        let read_fst: VectorFst<TropicalWeight> = read_text::<TropicalWeight, VectorFst<TropicalWeight>, _>(&mut cursor, None, None).unwrap();
    ...
    fn test_text_format_different_weights() {
        ...
        let mut cursor = Cursor::new(buffer);
        let read_fst: VectorFst<BooleanWeight> = read_text::<BooleanWeight, VectorFst<BooleanWeight>, _>(&mut cursor, None, None).unwrap();
        ...
        let mut cursor = Cursor::new(buffer);
        let read_fst: VectorFst<ProbabilityWeight> = read_text::<ProbabilityWeight, VectorFst<ProbabilityWeight>, _>(&mut cursor, None, None).unwrap();
...
mod binary_format_tests {
    ...
    fn test_write_read_binary_roundtrip() {
        ...
        fst.set_start(s0);
        fst.set_final(s2, TropicalWeight::new(3.14));
        ...
        // Read back from buffer
        let mut cursor = Cursor::new(buffer);
        let read_fst: VectorFst<TropicalWeight> = read_binary::<TropicalWeight, VectorFst<TropicalWeight>, _>(&mut cursor).unwrap();
    ...
    fn test_binary_format_efficiency() {
        ...
        for i in 0..99 {
            fst.add_arc(i, Arc::new(i + 1, i + 1, TropicalWeight::new(i as f32 * 0.1), i + 1));
        }
        ...
        println!("Binary size: {}, Text size: {}", binary_buffer.len(), text_buffer.len());
...
mod openfst_compat_tests {
    ...
    fn test_openfst_roundtrip_basic() {
        ...
        let read_fst: VectorFst<TropicalWeight> = read_openfst::<VectorFst<TropicalWeight>, _>(&mut cursor).unwrap();
